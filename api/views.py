...
def get_my_restaurants(request):
    ...
    if request.method == 'GET':
        restaurants = Restaurant.objects.filter(owner_id=employee.id) | \
                      Restaurant.objects.filter(employees__in=[employee])
...
def get_all_restaurants(request):
    try:
        ...
        return Response(serializer.data)
    if request.method == 'GET':
        restaurants = Restaurant.objects.exclude(owner_id=employee.id) | \
                      Restaurant.objects.filter(employees__in=[employee])
        serializer = RestaurantCreateListSerializer(restaurants, many=True)
        return Response(serializer.data)
